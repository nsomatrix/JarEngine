...
import org.je.app.classloader.ClassPreprocessor;
import org.je.app.classloader.ExtensionsClassLoader;
import org.je.app.classloader.InstrumentationConfig;
...
public class AppletProducer {
	...
	public static void createHtml(File htmlOutputFile, DeviceImpl device, String className, File midletOutputFile,
		...
		try {
			...
			writer.write("<html>\n");
			writer.write("\t<head>\n");
			writer.write("\t\t<title>JarEngine</title>\n");
			writer.write("\t</head>\n");
			writer.write("\t<body>\n");
			writer.write("\t\t<applet code=\"org.je.applet.Main\"\n");
	...
	public static void createMidlet(URL midletInputUrl, File midletOutputFile) throws IOException {
		...
		try {
			...
			URL url = AppletProducer.class.getResource("/je-injected.jar");
			if (url != null) {
				ijis = new JarInputStream(url.openStream());
				while ((jarEntry = ijis.getNextJarEntry()) != null) {
					if (jarEntry.getName().equals("org/je/Injected.class")) {
				...
				Logger.error("Cannot find je-injected.jar resource in classpath");
			}
		} finally {
	...
	private static String usage() {
		return "--midletClass {midlet class name} \n" +
			   "--appletInput {je-javase-applet.jar location} \n" +
