...
import org.je.app.classloader.ExtensionsClassLoader;
import org.je.app.classloader.MIDletClassLoader;
import org.je.app.classloader.MIDletClassLoaderConfig;
...
import org.je.log.StdOutAppender;
import org.je.microedition.ImplFactory;
import org.je.microedition.ImplementationInitialization;
import org.je.microedition.io.ConnectorImpl;
import org.je.util.Base64Coder;
...
public class Common implements MicroEmulator, CommonInterface {
    ...
    protected String saveJar2TmpFile(String jarUrl, boolean reportError) {
        ...
        try {
            ...
            if (systemTmpDir != null) {
                tmpDir = new File(systemTmpDir, "je-apps-" + MIDletSystemProperties.getSystemProperty("user.name"));
    ...
    private MIDletClassLoader createMIDletClassLoader(boolean forJad) {
        ...
        if (!Serializable.class.isAssignableFrom(Injected.class)) {
            Logger.error("classpath configuration error, Wrong Injected class detected. je-injected module should be after je-javase in eclipse");
        }
